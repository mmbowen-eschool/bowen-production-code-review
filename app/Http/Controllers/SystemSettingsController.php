...
class SystemSettingsController extends Controller {
    ...
    public function store(Request $request) {
        ...
        $settings = array(
            ...
            'display_currency', 'usd_exchange_rate', 'cny_exchange_rate',
            //            'currency_code','currency_symbol'
        );
        try {
            // Validate currency display settings
            $displayCurrency = $request->input('display_currency', 'MMK');
            if (!in_array($displayCurrency, ['MMK', 'CNY', 'USD'])) {
                $displayCurrency = 'MMK';
            }
            
            $usdRate = (float)($request->input('usd_exchange_rate') ?? 3500);
            if ($usdRate <= 0) {
                $usdRate = 3500;
            }
            
            $cnyRate = (float)($request->input('cny_exchange_rate') ?? 500);
            if ($cnyRate <= 0) {
                $cnyRate = 500;
            }
            ...
            foreach ($settings as $row) {
                if ($row == 'horizontal_logo' || $row == 'vertical_logo' || $row == 'favicon' || $row == 'login_page_logo' || $row == 'student_web_background_image') {
                    ...
                    // Handle currency display settings with validation
                    ...
                    } elseif ($row === 'usd_exchange_rate') {
                        $data[] = [
                            "name" => $row,
                            "data" => $usdRate,
                            "type" => "string"
                        ];
                    } elseif ($row === 'cny_exchange_rate') {
                        $data[] = [
                            "name" => $row,
                            "data" => $cnyRate,
                            "type" => "string"
            ...
            if (Auth::user()->hasRole('Super Admin')) {
                $this->systemSettings->upsert([
                    ...
                    ["name" => 'display_currency',
                     "data" => in_array($request->display_currency, ['MMK', 'CNY', 'USD']) ? $request->display_currency : 'MMK',
                     "type" => "string"
                    ],
                    ["name" => 'usd_exchange_rate',
                     "data" => max(1, (float)($request->usd_exchange_rate ?? 3500)),
                     "type" => "string"
                    ],
                    ["name" => 'cny_exchange_rate',
                     "data" => max(1, (float)($request->cny_exchange_rate ?? 500)),
                     "type" => "string"
                    ]
                ], ["name"], ["data"]);
