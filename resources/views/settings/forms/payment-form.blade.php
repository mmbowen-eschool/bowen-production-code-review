...
{{-- Currency Display Settings (for amount conversion display) --}}
<div class="border border-secondary rounded-lg mb-3">
    <h3 class="col-12 page-title mt-3 ">
        {{ __('Currency Display Settings') }}
    </h3>
    <div class="row my-4 mx-1">
        <div class="form-group col-sm-12 col-md-4">
            <label for="display_currency">{{ __('Display Currency') }}</label>
            <select name="display_currency" id="display_currency" class="form-control">
                <option value="MMK" {{ ($settings['display_currency'] ?? 'MMK') === 'MMK' ? 'selected' : '' }}>MMK (Myanmar Kyat)</option>
                <option value="CNY" {{ ($settings['display_currency'] ?? 'MMK') === 'CNY' ? 'selected' : '' }}>CNY (Chinese Yuan)</option>
                <option value="USD" {{ ($settings['display_currency'] ?? 'MMK') === 'USD' ? 'selected' : '' }}>USD (US Dollar)</option>
            </select>
            <small class="form-text text-muted">{{ __('Select currency for displaying amounts') }}</small>
        </div>

        <div class="form-group col-sm-12 col-md-4">
            <label for="usd_exchange_rate">USD Exchange Rate (1 USD = ? MMK)</label>
            <input type="number" name="usd_exchange_rate" id="usd_exchange_rate" 
                   value="{{ $settings['usd_exchange_rate'] ?? 3500 }}" 
                   min="1" step="1" class="form-control" required>
            <small class="form-text text-muted">{{ __('How many MMK per 1 USD') }}</small>
        </div>

        <div class="form-group col-sm-12 col-md-4">
            <label for="cny_exchange_rate">CNY Exchange Rate (1 CNY = ? MMK)</label>
            <input type="number" name="cny_exchange_rate" id="cny_exchange_rate" 
                   value="{{ $settings['cny_exchange_rate'] ?? 500 }}" 
                   min="1" step="1" class="form-control" required>
            <small class="form-text text-muted">{{ __('How many MMK per 1 CNY') }}</small>
        </div>
    </div>
</div>
