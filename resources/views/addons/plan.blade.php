                                ...
                                <div class="col-md-6 col-xl-3 grid-margin stretch-card pricing-card">
                                    <div class="card border-primary ribbon  border pricing-card-body">
                                        <div class="text-center pricing-card-head mb-2 text-center">
                                            <h4>{{ $addon->name }}</h4>
                                            <p>{{ __('price') }} : {{ format_money($addon->price) }} </p>
        ...
        setTimeout(() => {
            document.getElementById('razorpay-button-{{ $addon->id }}').onclick = function(e) {
                ...
                $.ajax({
                    ...
                    data: {
                        amount : $('.bill_amount-{{ $addon->id }}').val(), // Amount is in currency subunits. Default currency is MMK. Hence, 100 refers to 100 MMK
                    ...
                    success: function (response) {
                        if (response.data) {
                            ...
                            var options = {
                                ...
                                "amount": $('.bill_amount-{{ $addon->id }}').val() * 100, // Amount is in currency subunits. Default currency is MMK. Hence, 100 refers to 100 MMK
                                "currency": "{{ $system_settings['currency_code'] ?? 'MMK' }}",
